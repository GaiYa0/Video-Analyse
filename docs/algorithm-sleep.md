...
4. **直立门控**：头在脖子上方超过 **0.10 × 尺子**，或鼻+双眼都在且头仍在脖子上方 8px 以上 → 角度封顶 18°（低于恢复阈值，进不了低头）。真趴桌时头落到颈线或更低，门控不生效。
...
| `SLEEP_HOLD_MS` | **10000** | 连续低头达到此时长才可能报睡岗 |
...
| `MAX_HEAD_ABOVE_NECK_RATIO` | 0.10 | 头最多高出颈线这么多还能算低头 |

布控规则若带 `sleep_on_duty`：`thresholdMs` = 持续时间；`distanceThresholdPx` 复用为低头角度；`directionToleranceDeg` 复用为滞回差（默认 10）。选了 `on_sleep_pose` 但没下发规则时，Analyzer 会补一条默认规则（10 秒）。

**注意**：布控页现在会自动写一条 `sleep_on_duty`，持续时间**写死 2500ms**，会盖掉 Analyzer 的 10 秒默认值。要 10 秒就在布控页把持续时间改成 10000 再保存。页面默认值在 `web/`，归同学 C。滞回差在页面上没有输入框，恒等于 `低头角 − 10`。
...
| 有效帧 | ≥ **12** 帧真实姿态 | 靠丢帧和抖动凑出来的时长 |
| 丢帧占比 | 累计缺失 / 窗口 ≤ **0.35** | 中途被挡、人走开 |

告警前还有独立的第二道门（`isSleepOnDutyHit`）：`motionState != moving`、躯干速度 ≤ 8 px/s、轨迹年龄 ≥ 12 帧、`dwellMs` 覆盖整个窗口。

睡岗布控的告警视频前缀从 30 帧收到 **10** 帧，减少「已经趴下却还在等缓存」的延迟。原 YOLO 布控仍是 30 帧。

验收：本地视频里睡岗要出事件，点头/看键盘/看手机/正面看镜头/背景里的人都不能出。正拍、侧拍趴桌都应能过 32° 并在约 10 秒后变黄框。黄框文字是 `SLEEP <角度> <已持续秒数>`；成立那一刻 Analyzer 日志会打一行 `sleep_on_duty track=… peak=… downRatio=… drift=…`，用来解释为什么这次算数。
...
2. 布控选 **睡岗检测**（`on_sleep_pose` + `person` + 闭合区域）；行为规则里把**持续时间改成 10000ms**（页面自动写的是 2500）  
3. **不要**选启发式「睡觉」  
4. 录像引擎用 **算法服务器**；保存后到布控列表 **停止再启动**  
5. 推流画面：绿框=检出人，黄框写 `SLEEP <角度> <秒数>`（常带 `!`）=睡岗成立  
6. 告警看 **告警管理**，类型 **睡岗** / `SLEEP_ON_DUTY`；布控页「最近事件」在推流模式下经常是空的  
7. 反例要一条条走：打字看资料 10 秒、低头看手机、正脸看镜头、走开留空座位、背景里站个人——都不该出告警  
...
持续时间提到 10 秒，并加了帧质量门、几何硬条件和窗口证据（占空比 / 峰值 / 静止 / 丢帧预算）。单元测试 33 条，含每类误报一条反例和一条 Python↔`SleepPose.h` 常量对齐测试——以后只改一边会直接测试失败。

代价是**漏报变多**：演示时要在画好的区域里、头明确低下、别乱动，撑满 10 秒。轨迹丢失超过 4 秒会被回收，计时从头开始。
