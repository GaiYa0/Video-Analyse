...
| 头点静止 | 相对锚点最大漂移 ≤ **0.45 × 尺子**（峰值角 ≥ **90°** 时不启用） | 看手机、写字（50–70° 还在动）。已经趴实（90–135°）时，循环片源/继续趴下造成的头点跳动不再挡告警 |
...
同一个目录还会写一张 **`keyframes.jpg`**：把「起始 / 峰值 / 结束」三帧横向拼成一张对比图（`GenerateAlarmVideo::writeKeyframeStrip`）。峰值帧取 `pitchDegree` 最大的那一帧。这张图比整段视频更适合放进邮件和答辩材料——一眼能看清头是怎么趴下去的。
...
展开页序、优化点、不要写的话：[答辩PPT要点-优化与创新.md](./答辩PPT要点-优化与创新.md)。数字仍以本文件为准：
