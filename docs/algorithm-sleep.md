...
```text
H.264 流（直连 live/{apeId} 或国标 rtp/{设备_通道}，只换 URL）
  → 原管线解码（不改）
...
国标只换 Analyzer 打开的 `streamUrl`（`rtsp://…/rtp/<设备_通道>`），**不改**俯仰角、5 秒保持、窗口证据。阈值仍以本文 §2–4 与 `SleepPose.h` 为准。C 写 PPT/视频时用这些数，不要另编一套国标阈值。

演示机须先完成国标原 YOLO（[gb28181-yolo-verify.md](./fixtures/gb28181-yolo-verify.md)），再：

1. 同一国标设备布控 `on_sleep_pose` + `person` + 闭合区域；趴桌至 `BOW` 后再稳住约 5 秒，应报 `SLEEP_ON_DUTY`。
2. 直连工位 / webcam 再布控一次睡岗，仍应能报（公式相同）。
3. 直连或国标上再开一条原 YOLO，确认没拆掉。
4. 不要选启发式「睡觉」。

本机 WSL24 无 WVP 时只做直连回归；国标睡岗跟 A 要演示机窗口。不要改 `docs/当前阶段.md` 的 `phase`。
