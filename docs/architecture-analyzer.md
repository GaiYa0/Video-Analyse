...
拉流默认 `nobuffer` / `low_delay`，包队列只留 **4** 帧作解码突发缓冲。解码线程发现队列里还有更新的包、且刚推理过时，只跳过 YOLO，BGR 和告警队列仍按解码帧写入，避免证据 MP4 只剩一帧。回推编码队列只留 **1** 帧，时间戳按墙钟、时长按相邻帧间隔，GOP 约 5 帧、不编 B 帧。布控预览可以丢帧；告警视频不行。前端直播 FLV 必须把 `enableStashBuffer: false` 传给 flv.js 的 **Config（第二个参数）**，否则浏览器缓冲会一直涨。
...
现网 `detect.event` 会先插入告警行（封面用 start 截图，并带预设 `video_path`）。Analyzer 再把 mp4 写到同一路径。睡岗时叠加 `behavior_type=sleep_on_duty`、`alarmType=SLEEP_ON_DUTY`、`customEventName=睡岗`，以及 `confidence` / `pitchDegree` / `durationFrames` / `duration_ms` / `sleepLevel` / `sleepScore`（见 [algorithm-sleep.md](./algorithm-sleep.md)）。`sleepLevel` 为 0/1/2，对应 2s 疑似、5s 确认、15s 严重三档；`sleepScore` 为 0–100 质量分，仅用于排序。同一事件内两者都只升不降，backend 据 `sleepLevel` 写 `alarm_level` 与分档类型名。编码失败会打 `genAlarmVideo failed`，不再静默丢掉。
