...
# 把库里的 ws://127.0.0.1:9992/live/... 改成同学能打开的
# ws://<局域网IP>:8080/live/...
# 用法：
...
echo "rewrite play_url to ws://${LAN_IP}:8080/live/..."
Q "update h_device
     set play_url = concat('ws://${LAN_IP}:8080/live/', ape_id, '.live.flv')
   where play_url like 'ws://127.0.0.1:9992/live/%'
...
echo "h_device:"
Q "select ape_id, name, play_url from h_device;"
echo "h_screen_wall_stream:"
Q "select id, title, play_url from h_screen_wall_stream;"
echo "done. classmates: force-refresh http://${LAN_IP}:8080/"
