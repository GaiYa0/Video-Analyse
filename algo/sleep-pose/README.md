...
`update_temporal()` 返回的 `FrameDecision.sleep_level` 即档位（`-1` 未低头 / `0` 疑似 / `1` 确认 / `2` 严重），与 Analyzer 上报的 `sleepLevel` 同义。`FrameDecision.sleep_score` 是 0–100 质量分，见 `sleep_quality_score()`，同样不参与判定。
